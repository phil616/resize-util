...
## 体积优先：无损格式压不动就自动转 JPEG

本工具**只关心大小**：PNG/BMP/TIFF 这类无损格式只能靠降分辨率压小，复杂图往往
压不到目标。这种情况会**自动转成 JPEG** 来达成目标体积（输出 `*.jpg`，不在乎质量
与透明通道）：

- `photo.png --target 100K` → 自动产出 `photo.min.jpg` ≈ 100K；
- 仅当无损格式**确实能**压到目标（如很大的目标）才保留原格式；
- `--inplace` 同样适用：会把原 `.png` 替换为 `.jpg` 并删除原文件；
- 固定质量模式（无 `--target`）下，无损结果若压不过原图，也会退回 JPEG。

若就是要保留无损格式，可调低 `--min-scale`（如 `0.05`）让它靠更狠的降分辨率压到
目标；固定质量模式下还可加 `--allow-larger` 强行写出同格式结果。
